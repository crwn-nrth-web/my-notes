...
#### Ptolemaic model 
The model combined two devices to account for observed planetary motion proposed by [[Ancient-astronomy#Hellenistic period|Aristarchus of Samos]]:
...
![[Pasted image 20260603135526.png]]
...
He says that it is true that what happens in the heavens would look the same if the earth rotated rather than the heavens rotating, but what happens on earth would look vastly different. His argument is that 
- throwing an object in the air would seem to travel towards the west. 
- Even if the air moves with the earth, the object would still travel towards the west
- if the object was fused with the air, then the object would appear still even if the object was thrown
...
> For first, the mathematicians are so uncertain of the motion of the Sun and Moon [in the geocentric model] that they cannot represent or even be consistent with the constant length of the seasonal year. Secondly, in establishing the motions both of the Sun and Moon and of the other five wandering stars they do not use the same principles or assumptions, or explanations of their apparent revolutions and motions. For some use only

#### objections to the Ptolemaic model
One problem at the time that Copernicus addressed is the length of the year. The length of a year is important because if the length of the year is incorrect, the calendar year would become out of sync with the natural year. The Ptolemaic model had a year being 365.25 days, which was adapted by the Julian calendar of 365.25 days. The Julian calendar added a leap day every four years but this caused The Julian calendar becoming out of sync with the natural year by 10 days and so in 1582 (after Copernicus death), the Gregorian calendar was adapted which reduced the number of leap days.

Copernicus sets a narrower definition of uniform circular motion than Ptolemy's definition. Uniform circular motion according to Ptolemy is that equal angles sweep out at equal times, and so his equant would satisfy circular motion. Copernicus argues that Ptolemy' equant violated uniform circular motion as it needs to be equal angles from the geometric center.

Another problem with Ptolemy for Copernicus is that Ptolemy' model for each planets were separate from one another, each planet having their own equant, etc. Copernicus equates to that of a monster rather than a man. 
...
- argues that the earth rotates and moves around the sun (against popular opinions)

Copernicus argues that since the earth is spherical, it too should follow the motion of spherical bodies, which is spherical
- this is an inconsistency in his model as the cosmos is spherical but it does not "move spherically" 
- did he justify his argument?

> Surely Aristotle's division of simple motion into three types, away from the middle, towards the middle, and round the middle, will be regarded merely as an intellectual division; just as we distinguish between a line, a point and a surface, although one cannot exist without the other, and none of them without a body . . .   I also add that it would seem rather absurd to ascribe motion to that which contains and locates, and not rather to that which is contained and located, that is the Earth. Lastly, since it is evident that the wandering stars are sometimes nearer, sometimes further from the Earth, this also will be an example of motion of a single body which is both round the middle, by which they mean the centre, away from the middle, and towards it. Motion round the midpoint must therefore be accepted more generally, and as satisfactory, provided that each motion is motion about its own midpoint. You see then that from all these arguments the mobility of the Earth is more probable than its immobility, especially in the daily revolution, as that is particularly fitting for the Earth.(p. 45)

His argument that the earth should move is based on probability that everything rotates so the 
The problem that Copernicus is coming up against is that Aristotelian phyiscs is the prevailing theory. Copernicus's model no longer can justify how anything moves (why does water etc. travel towards the center of the earth and not the center of the universe). He proposes the heliocentric model but he does not have a physics that goes along with (that is given by Newton). So, Copernicus was not able to justify his argument that earth should move.
